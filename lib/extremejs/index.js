...
exports.httpfunc = function(req, res) {
  ...
  if(req.method.toLowerCase() == 'post' && req.url.toLowerCase() == '/upload'){
    require('./img').cropImg(req, res, function(rs){
      res.writeHead(rs.code);
      if(rs.slice) res.write(JSON.stringify(rs.slice));
      res.end();        
    });
    return;
  }
